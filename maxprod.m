...
LowerBoundLogZ = appxOfphi + appxOfclique + BetheFreeEntropy;
    ...
    %%%%%% find MAP assignment
    MAPassignment = zeros(nodeNum, 1);
    maxVal = zeros(nodeNum, 1);
    for i = 1:nodeNum
...
function scaledMsg = updateMessage(i, j, xj, A, phi, oldMsg, scale)
    ...
    msgSum = zeros(size(phi,1), 1);
    indx=1;
    for xi = 1:rowsA
        msgSum(indx,1) = exp( phi(xi,i) ) * Indicator(i, j, xi, xj, phi) * getNeighborMsg(i, j, xi, oldMsg, A);
        indx += 1;
    end

    scaledMsg = max(msgSum)*scale;
end
